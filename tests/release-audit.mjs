...
const excludedPrefixes = ['node_modules/', 'policy-engine/build/', 'crm/build/', 'runtime/', 'artifacts/runtime/', 'backups/'];
const pathLeakEvidencePrefixes = ['evidence/crm/'];
...
for (const file of files) {
  ...
  if (!binaryExtensions.has(extname(rel).toLowerCase())) {
    ...
    if (absoluteLeak.test(text) && !pathLeakEvidencePrefixes.some((prefix) => rel.startsWith(prefix))) leakedPaths.push(rel);
  }
}
